...
int main()
{
    ...
    while (repeat == 1)
    {
        ...
        { // Input validation, If option is not either 1 or 2 displays errors and prompts user again
            cout << endl
                 << "Error ... Incorrect option. Try Again" << endl
                 << endl;
            cout << "Choose one of the following options" << endl;
        ...
        case 1:
            ...
            if (!infile)
            { // Input validation, if user input file does not exist displays error and breaks out of switch and returns user to main menu
            ...
            cout << courseID + " " << dash + " " << intro + " " << to + " " << c + " " << program + " " << endl
                 << endl;
            cout << setw(20) << left << prof + " " + profFName + " " + profLName << setw(19) << right << "Term: " << term + " " << year << endl
                 << endl
                 << endl;

            cout << "List of Students" << endl
                 << headerLine; // Formatting

            while (infile >> firstName >> lastName >> grade)
            { // Loop that reads the data from the input file for the student names and grades, terminates loop once all data is read
                cout << endl
                     << setw(30) << left << firstName + " " + lastName << left << grade; // Formatting
                if (grade > gradeMax)
                { // Checks if read grade is greater than current value of gradeMax, if so updates the value to new higher grade
                    gradeMax = grade;
                }
                if (grade < gradeMin)
                { // Checks if read grade is lesser than current value of gradeMin, if so updates the value to new lower grade
                    gradeMin = grade;
                }
        ...
        case 2:
            cout << endl
                 << "Good Bye ..." << endl; // Displays goodbye message
